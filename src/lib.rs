...
#[allow(unused)]
impl<D> ArenaTree<D> where D: Clone + Debug + Default + PartialEq {
    #[inline(always)]
    ...
    pub fn node_ref(&self, idx: NodeIdx) -> TreeResult<&Node<D>> {
        self.nodes.get(idx.0).ok_or(TreeError::NoNodeForNodeIdx { idx })
    }

    #[inline(always)]
    pub fn node_mut(&mut self, idx: NodeIdx) -> TreeResult<&mut Node<D>> {
        self.nodes.get_mut(idx.0).ok_or(TreeError::NoNodeForNodeIdx { idx })
    }

    pub fn new_node<P>(&mut self, parent: P) -> TreeResult<NodeIdx>
    where P: Into<Option<NodeIdx>> {
        let data = D::default();
        if let Some(cidx) = self.garbage.pop_front() {
            ...
            self.nodes.push(Node { idx: cidx, parent, children: vec![], data });
    ...
    fn destroy_node(&mut self, idx: NodeIdx) -> TreeResult<()> {
        ...
        if let Some(pidx) = self.node_ref(idx)?.parent {
            ...
            parent.children = parent.children.drain(..)
...
impl<D> fmt::Display for ArenaTree<D> where D: Clone + Debug + Default + PartialEq {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        for node_idx in self.dfs(NodeIdx::ROOT).unwrap(/*TreeResult*/) {
            ...
            let num_ancestors: usize = self.ancestors_of(node_idx)
                .unwrap(/*TreeResult*/)
                .count();
            for _ in 0 .. num_ancestors { write!(f, "| ")?; /* no newline */ }
...
where D: Clone + Debug + Default + PartialEq + Serialize {
...
where D: Clone + Debug + Default + PartialEq + Deserialize<'de> {
    fn deserialize<DE: Deserializer<'de>>(d: DE) -> Result<Self, DE::Error> {

        #[derive(Deserialize)]
        #[serde(field_identifier, rename_all = "lowercase")]
        enum Field { Nodes, Garbage }
        ...
        where D: Clone + Debug + Default + PartialEq + Deserialize<'de> {
            ...
            where V: SeqAccess<'de> {
                let nodes = seq.next_element()?
                    .ok_or_else(|| de::Error::invalid_length(0, &self))?;
                let garbage = seq.next_element()?
...
where D: Clone + Debug + Default + PartialEq
...
where D: Clone + Debug + Default + PartialEq + Core;




#[derive(Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
    #[derive(deltoid_derive::Delta, Deserialize, Serialize)]
pub struct Node<D> {
    idx: NodeIdx,
...
impl<D> Node<D> {
    #[inline(always)]
    pub fn parent(&self) -> Option<NodeIdx> { self.parent }
    ...
    pub fn count_children(&self) -> usize { self.children.len() }
    ...
    #[inline(always)]
    pub fn node_idx(&self) -> NodeIdx { self.idx }

    #[inline(always)]
    pub fn data_ref(&self) -> &D { &self.data }
